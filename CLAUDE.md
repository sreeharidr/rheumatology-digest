...
2. **SEO work not yet done** (from the Aug 2026 Search Console review): title tags — 28 of 38 titles exceed 50 chars, so the `| Rheumatology Digest` brand suffix is truncated away in SERPs (fix by shortening titles; slugs are explicit so retitling breaks no URLs); WhatsApp CTA moved above the fold with a QR code for desktop; mobile-only sticky bottom bar (**not** a modal popup — Google penalises intrusive mobile interstitials); author byline + credentials on post pages for E-E-A-T (note: `hideAuthor = true` is currently set site-wide at the user's request, so scope any byline to post pages only); citation/DOI block moved near the top; `isBasedOn` in the Article schema.
3. **Distribution, not just SEO.** Traffic is the binding constraint, not CTR — ~9 impressions/day means CTR fixes alone cannot move WhatsApp numbers. *(Done Aug 2026: the infographic template now burns a `rheumatologydigest.org` footer strip into every image, so a forwarded image carries a route back to the site. A WhatsApp-channel QR in the same strip is the remaining optional upgrade — the domain alone requires the reader to type it.)* Remaining items: add evergreen *clinical-question* posts alongside the trial-specific ones (trial posts rank easily but almost nobody searches a trial acronym; fibromyalgia outperformed everything because the topic has real volume); auto-post new items from RSS via Buffer/Make (Instagram and Facebook fit the WhatsApp-native target markets and the 9:16 infographic; **skip X** — API is paid and organic reach was poor). Adding an image `<enclosure>` to the RSS feed would let those tools attach the infographic directly.
4. **Tag-based vector icons** instead of cover images on list pages — visual differentiation, requires SVG design work.
5. **`rdpost` shell helper** — wraps `hugo new` + `open` to skip Finder navigation when creating posts. Defer until folder navigation actually feels slow.
6. **Bump GitHub Actions to Node 24** — `actions/checkout@v4`, `configure-pages@v5`, `upload-artifact@v4`, `deploy-pages@v4` are flagged for forced Node 24 from June 2, 2026. Update versions when convenient.
7. **Brand theming** — colors (navy `#1E3A5F`, off-white `#F5F3EE`), Inter font, flat SVG iconography. Step 7 of the original roadmap.
8. **Remaining content sections** — quizzes (`content/quizzes/`) and learning modules (`content/modules/`) will be sibling sections to `content/posts/` and `content/cases/`, each with its own top-nav entry and layout. Different from the research/reviews split (a category WITHIN posts) — these are entirely separate content types. *Cases (`content/cases/`) launched May 2026 — see the "Per-case workflow" section above.*
9. **`www` TLS cert** — GitHub Pages' Let's Encrypt cert currently only covers the apex; `https://www.rheumatologydigest.org/` throws a cert warning. HTTP-www redirects fine to apex. Fix when convenient by removing/re-adding the custom domain in repo Settings → Pages, which forces a cert reissue covering both forms.
