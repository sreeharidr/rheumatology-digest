...
4. **Write the body** — TL;DR blockquote at top, then the user's clinical summary, ending with `{{< source >}}`. Voice rules:
   - **Do not trim. Reproduce the user's summary in full** (standing instruction, Aug 2026 — supersedes all earlier length guidance). There is **no word-count or reading-time target, and no editorial cutting**. The user's pasted text is the post. Do not shorten it, do not drop sections, do not "compress" enumerations into tables, and do not spend any time on the reading-time figure. A 3,500-word, 17-minute post is a correct outcome.
     - **What Claude still does** is everything *around* the text: front matter (title, `summary`, `description`, tags, categories, `source`), the TL;DR blockquote, and formatting for the web — converting the user's `> **1. SECTION**` blockquote-headings into real `## ` headings so PaperMod builds a table of contents, fixing markdown tables that won't render, and dropping the pasted "ARTICLE INFORMATION" header block because front matter plus `{{< source >}}` already renders that citation.
     - **The one thing still removed** is meta-commentary addressed to the user rather than to readers — see the bullet below. Where the framing is second-person but the content is reader-facing (e.g. "a caution the fellow should register"), reword the framing and keep the content rather than cutting it.
     - History, so it isn't re-litigated: posts up to Aug 2026 ran 2,500–3,000 words, which the user briefly judged too long; guidance became "~800–1000 words / 4–5 min", which got over-applied and cost real substance; it was then relaxed to "cut boring, not long"; the user then removed the editorial discretion entirely with the rule above. **Do not reintroduce a length target.**
