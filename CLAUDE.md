...
│   └── whatsapp-qr.svg             # Committed static QR for the WhatsApp channel — generated offline, never fetched from a QR API. INLINED into the page by `readFile`, not linked as <img src>. Regeneration command in "Tooling notes".
...
- **`absURL` bakes in the production `baseURL`, so local previews of built output request the live domain.** `{{ "whatsapp-qr.svg" | absURL }}` emits `https://rheumatologydigest.org/whatsapp-qr.svg` — correct in production, but any local view of `public/` then fetches the *live* site, and an asset that has not been deployed yet 404s and renders as a broken image. This is doubly confusing because `hugo server` rewrites `baseURL` to localhost, so the same page looks fine on the dev server and broken the moment you check the built output (or the dev server stops and something else serves `:1313`). It cost a "why is the QR broken?" round-trip. Use `relURL` for site assets, or — better for small assets — inline them with `readFile` and skip URL resolution entirely.
...
User's offline workflow: select published paper → make a portrait HTML infographic → previously shared to WhatsApp. The user supplies the paper and the infographic; **Claude writes the summary** (Sep 2026 change — see step 4). The site adds a permanent record; WhatsApp now shares the post URL (infographic appears as link preview).
...
3. **Drop the infographic** into the same folder as `infographic.png`. The archetype already references this filename in `cover.image`. Since Aug 2026 the user's infographic template burns a **`rheumatologydigest.org` footer strip** into every image, so a forwarded WhatsApp image still points back to the site — don't crop or regenerate images in a way that loses it.
   - **The user compresses infographics with `pngquant`, and the compressed file is the one to ship.** Two consequences, both of which have caused problems. (a) The export sometimes lands with pngquant's default suffix — `infographic-fs8.png` — which does **not** match `cover.image`, so `og:image` 404s and every WhatsApp/LinkedIn preview silently breaks. Rename it to `infographic.png`. (b) The file often changes **after** you copy it into the bundle, because compression happens as a separate step: the Sep 2026 lung-disease post went 1.9 MB → 3.9 MB between the copy and the commit. **Re-check the bundle image immediately before `git add`** — don't trust a hash you verified earlier in the turn. A larger uncompressed file sitting in the archive folder is usually the *pre*-pngquant original and is not what should be pushed.

4. **Write the body** — TL;DR blockquote at top, then the clinical summary, ending with `{{< source >}}`. Voice rules:
   - **Claude writes the summary** (standing instruction, Sep 2026 — supersedes the earlier "the user writes their own summary" rule). Claude reads the source PDF from the archive folder and drafts the full clinical write-up. **The user reviews and edits before push** — that review is now the quality gate, so never push a Claude-drafted body without the user having seen it.
   - **No length target, and no editorial cutting** (standing instruction, Aug 2026 — still in force, and it now governs Claude's own drafting). Cover what the paper covers. Mirror its structure, keep its enumerations as enumerations, and do not "compress" sections to save the reader time. A 3,500-word, 17-minute post is a correct outcome. Do not spend any time on the reading-time figure.
     - History, so it isn't re-litigated: posts up to Aug 2026 ran 2,500–3,000 words, which the user briefly judged too long; guidance became "~800–1000 words / 4–5 min", which got over-applied and cost real substance; it was then relaxed to "cut boring, not long"; the user then removed the editorial discretion entirely. **Do not reintroduce a length target.**
   - **Register: clinical teacher, third person.** Write for a practising rheumatologist who has not read the paper. State what the source says and what follows clinically from it. **Do not write first-person clinical experience** — no "in my practice", no "I've found", no invented case anecdotes. The site carries no byline, so the voice is the site's, not a claimed personal one.
   - **Attribute, don't assert beyond the source.** Every clinical claim in the body must be traceable to the paper being summarised. If something is worth saying but the paper does not support it, leave it out rather than reaching for background knowledge — an unsourced claim in a post with a DOI at the bottom reads as though the paper said it.
   - **Copyright rules bind Claude's drafting exactly as they bound the user's** (see the copyright posture under "Conventions"). Own-words summaries of facts only: **never copy the paper's sentences**, and never reproduce its figures or tables — including as the basis for an infographic. Open access is not permission; several sources are CC BY-NC-ND.
   - **TL;DR:** one sentence (max two), capturing the paper's thesis.
   - **Structure for the web:** real `## ` headings so PaperMod builds a table of contents, markdown tables that actually render, and no pasted "ARTICLE INFORMATION" citation block — front matter plus `{{< source >}}` already renders that.
   - **If the user does paste their own summary instead**, that text is the post: reproduce it in full, do not trim it, and apply only the web formatting above. The one thing still removed is **meta-commentary addressed to the user rather than to readers** — "teaching notes", "ask your fellows to…", discussion prompts, offers to build flowcharts/decks, or references to "this series" (meaning the Claude conversation). **Omit these silently; do not adapt them into reader-facing prose.** Adding one to the ADVANCE OUTCOMES/ralinepag post was wrong and had to be removed. Where the framing is second-person but the content is reader-facing (e.g. "a caution the fellow should register"), reword the framing and keep the content.
...
- **Published posts are an archive — do not retroactively edit them as the evidence moves** (standing instruction, Sep 2026). A post records what was written on its date, and it stays that way. **Do not** go back to add cross-references to newer posts, soften a conclusion a later paper undercut, or note that a drug's standing has since changed. Claude has twice offered to do exactly this and been told no; the answer is no by default. *Example:* the Aug 2026 IgA vasculitis post describes avacopan without the Sep 2026 news that its approval is being withdrawn — that post is not wrong, it accurately reported its source, and it was deliberately left alone. **The only exception is a genuine blunder**: a factual error that could mislead clinically (a wrong dose, a misstated result, a reversed conclusion). Evolving evidence is not a blunder. Note this rule is about *body content* — the Aug 2026 backfill of `description` and `title` across the archive was metadata for SEO and remains entirely fine to do.
...
- **Claude drafts, the user signs off.** Since Sep 2026 Claude writes the clinical summary itself from the source PDF (see step 4 of the per-post workflow). Clinical accuracy is still the value of this site, so the user's review before push is the gate that replaces their authorship — present the draft, wait for their edits, and do not treat "draft written" as "ready to push".
- **External archive:** `~/Documents/Rheumatology Digest/` (separate from this repo, NOT in git). Each paper lives in a dated subfolder containing the source PDF (+ eventually the infographic). Claude reads the PDF from there and drafts the summary → posts go in this repo.
...
- **`qrencode`** (Homebrew) generates `static/whatsapp-qr.svg` offline. To regenerate after a channel URL change:
  ```sh
  qrencode -t SVG -l M -m 4 -s 1 -o /tmp/qr.svg 'https://whatsapp.com/channel/<id>'
  ```
...
1. ~~**Backfill `description` on the remaining posts.**~~ **Done Aug 2026** — all 45 post pages now carry a purpose-written meta description ≤155 runes. `driver.mjs audit` enforces this going forward and is now green across the whole archive.
2. ~~**Desktop QR block.**~~ **Done Sep 2026** — `layouts/partials/whatsapp-qr.html` + `static/whatsapp-qr.svg`, shown only ≥768px, injected after the TL;DR by JS so no published post needed editing. See the convention bullet under "Conventions" for the design constraints.
3. **Remaining SEO items** (Aug 2026 Search Console review). ~~Title tags~~ **done Aug 2026** — all 27 over-length titles shortened to ≤50 chars so the `| Rheumatology Digest` suffix survives in SERPs; `driver.mjs` warns on any new title over 50. Still open: author byline + credentials on post pages for E-E-A-T (see the no-byline convention above — this is a *decision to revisit*, not a settled task; also needs the user's Google Scholar URL); citation/DOI block moved near the top of the post; `isBasedOn` in the Article schema (`schema_json.html` already emits a `Person` author, so enrich rather than duplicate).
