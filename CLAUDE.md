...
4. **Write the body** — TL;DR blockquote at top, then the user's clinical summary, ending with `{{< source >}}`. Voice rules:
   - **Length: keep it short.** Target **~800–1000 words / 4–5 min read**, roughly 5–7 `##` sections. Posts up to Aug 2026 ran 2,500–3,000 words (14 min) and the user judged that too long for how people read now. The Aug 2026 cutaneous-lupus post (834 words, 4 min) is the reference example. **The compression technique that works: convert enumerations into a markdown table** — one table replaced ~6 paragraphs there. Keep the TL;DR, the headline numbers, the caveats/limitations (the honest reading is the site's differentiator) and the practical algorithm; cut exhaustive methods detail, trial-by-trial history, and every subgroup.
